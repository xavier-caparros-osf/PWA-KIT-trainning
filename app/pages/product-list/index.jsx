...
const ProductList = (props) => {
    ...
    const [productSearchResultState, setProductSearchResultState] = useState({
        hits: [],
        offset: 0,
        baseLimit: 0
    })
    const basket = useBasket()
    const [choosenProduct, setChoosenProduct] = useState(undefined)
    ...
    useEffect(() => {
        if (productSearchResult) {
            setProductSearchResultState({
                hits: productSearchResult.hits,
                offset: productSearchResult.offset,
                baseLimit: productSearchResult.limit
            })
        }
    }, [productSearchResult])
    ...
    const showNoResults = !isLoading && productSearchResultState && !productSearchResultState?.hits
    ...
    const loadMore = async () => {
        searchParams.offset = productSearchResultState.offset + productSearchResultState.baseLimit

        searchParams.refine = ['htype=master']
        if (categoryId) searchParams.refine.push(`cgid=${categoryId}`)

        try {
            const newProductSearchResult = await api.shopperSearch.productSearch({
                parameters: searchParams
            })

            setProductSearchResultState({
                baseLimit: productSearchResultState.baseLimit,
                offset: searchParams.offset,
                hits: [...productSearchResultState.hits, ...newProductSearchResult.hits]
            })
        } catch (e) {
            console.log('error', e)
        }
                ...
                <>
                    ...
                    <Grid templateColumns={{base: '1fr', md: '280px 1fr'}} columnGap={6}>
                        ...
                        <Box>
                            ...
                            >
                                {isLoading || !productSearchResultState
                                    ? new Array(searchParams.limit)
                                          .fill(0)
                                          .map((value, index) => (
                                              <ProductTileSkeleton key={index} />
                                          ))
                                    : productSearchResultState.hits.map((productSearchItem) => {
                            ...
                            >
                                {productSearchResult &&
                                    productSearchResultState.hits.length <
            ...
            <Box>
                {isQuickViewOpen && (
                    <ProductViewModal
                        ...
                        addToCart={(variant, quantity) => handleUpdateCart(variant, quantity)}
                    />
                )}
            </Box>
        </Box>
...
ProductList.getProps = async ({res, params, location, api}) => {
    ...
    return {searchQuery: searchQuery, productSearchResult, isSearch, categoryId}
}
